...
// Route path for the truck to follow
const ROUTE_PATH_D =
...
// --- UPDATED LOGS (Narrative Arc: Resilience) ---
const SYSTEM_LOGS = [
  { t: 12, msg: 'AVISO: Pérdida de señal satelital (Cloud).', type: 'warning' },
  { t: 13, msg: 'HARDWARE: Modo Soberano activado. Grabación local.', type: 'action' }, // The Resilience Pivot
...
export default function RansaResearchConsole() {
  ...
  const phase: Phase = useMemo(() => {
    ...
    if (seconds < 15) return 'BLIND'; // 12s-15s: Cloud is dead, but System is FINE (Green)
    return 'INCIDENT'; // 15s+: System goes Red
  }, [seconds]);

  const isOffline = phase === 'BLIND' || phase === 'INCIDENT'; // Affects the Cloud Badge
  ...
  const Header = () => (
    <header className="flex h-16 items-center justify-between border-b border-white/10 bg-[#080808] px-6 shadow-md shrink-0 z-20">
      ...
      <div className="flex items-center gap-4">
         {/* CLOUD STATUS BADGE - Fails at 12s */}
         <div className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-bold tracking-widest transition-colors duration-500 ${
           isOffline 
             ? 'border-gray-700 bg-gray-800/50 text-gray-400' // Cloud goes grey/offline
             : 'border-blue-500/30 bg-blue-900/10 text-blue-400'
         }`}>
           {isOffline ? (
             <><CloudOff className="h-3.5 w-3.5" /> NUBE: OFFLINE</>
           ) : (
             <><Cloud className="h-3.5 w-3.5" /> NUBE: ONLINE</>
           )}
         </div>

         {/* SYSTEM STATUS BADGE - Remains GREEN during Blind Zone, Red only on Incident */}
         <div className={`flex items-center gap-2 rounded-full border px-4 py-1.5 text-xs font-bold tracking-widest transition-colors duration-500 ${
           isSystemAlert 
             ? 'animate-pulse border-red-500 bg-red-900/30 text-red-400' 
             : 'border-emerald-500/30 bg-emerald-900/10 text-emerald-400'
         }`}>
           {isSystemAlert ? (
             <><AlertTriangle className="h-4 w-4" /> SISTEMA: ALERTA</>
           ) : (
             // Here is the KEY differentiator: System stays NOMINAL even if Offline
             <><Cpu className="h-4 w-4" /> SISTEMA: NOMINAL</>
           )}
         </div>
      </div>
    </header>
             ...
             <svg viewBox="0 0 900 520" className="h-full w-full overflow-visible">
                ...
                {isOffline && (
                   ...
                   <>
                     ...
                     <text x="550" y="150" textAnchor="middle" className="fill-orange-500/50 text-[10px] font-mono tracking-[0.1em]">COBERTURA CLOUD: 0%</text>
                   </>
                )}
                ...
                {/* Cities */}
                {[
                  { x: 720, y: 110, name: "CUSCO", alt: "3400 m" },
                  { x: 460, y: 280, name: "NAZCA", alt: "520 m" },
                  { x: 150, y: 460, name: "LIMA", alt: "0 m" }
                ].map(city => (
                  <g key={city.name}>
                ...
                <motion.g
                   ...
                   style={{ filter: isSystemAlert ? 'drop-shadow(0 0 14px rgba(239,68,68,0.75))' : 'drop-shadow(0 0 12px rgba(16,185,129,0.7))' }}
                >
                   {/* Pulse Effect */}
                   <motion.circle 
                      r={40} 
             ...
             <div className="absolute bottom-8 left-8 right-8 flex flex-col gap-3">
                ...
                <div className="rounded-xl border border-white/10 bg-black/70 p-3 shadow-lg shadow-emerald-500/10 backdrop-blur">
                  <div className="flex items-center gap-3 text-xs uppercase tracking-widest text-gray-400">
                    <span className="text-[10px] font-bold text-emerald-300">{isSystemAlert ? 'RUTA EN MODO EMERGENCIA' : 'RUTA EN CURSO'}</span>
