...
export default function RansaResearchConsole() {
  ...
  // Map Movement Logic
  const [truckPos, setTruckPos] = useState({ x: 750, y: 100, angle: 0 });
  const smoothJourney = useMotionValue(journeyProgress);
  const [currentJourneyPos, setCurrentJourneyPos] = useState(journeyProgress);
  ...
  useEffect(() => {
    ...
    const dist = Math.min(currentJourneyPos * len, len - 0.1); 
    
    const point = el.getPointAtLength(dist);
    // Look ahead slightly for angle calculation
    const nextPoint = el.getPointAtLength(Math.min(dist + 1, len)); 
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="relative flex-[1.8] overflow-hidden border-r border-white/10 bg-gradient-to-br from-[#050b10] via-[#030509] to-[#010102]">
          ...
          <div className="absolute inset-0 p-4">
            ...
            <MapOverlayUI isOffline={isOffline} isSystemAlert={isSystemAlert} journeyPosition={currentJourneyPos} />
          </div>
        </div>
